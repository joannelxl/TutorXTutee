  ...
  async mounted() {
    ...
    onAuthStateChanged(auth, async (user) => {
      if (user) {
        this.user = user;
        const q1 = query(
          collection(db, "TutoringArrangements"),
          where("tuteeEmail", "==", this.user.email),
          orderBy("tutorEmail")
        );
        ...
        if (size > 0) {
          ...
          (await getDocs(q1)).forEach(async (document) => {
            ...
            var account = await getDoc(
              doc(db, "Tutors", document.data().tutorEmail)
            );
            arrangement.tutorName =
            ...
            const q2 = query(
              collection(db, "Chats"),
              where("TuteeEmail", "==", this.user.email),
              where("TutorEmail", "==", document.data().tutorEmail)
            );
    ...
    <div id="arrangements">
      <div
        ...
        <div class="information">
          ...
          <text style="font-weight: bold">Level: </text>
          {{ arrangement.level }} <br />
          <text style="font-weight: bold">Subject: </text>
          {{ arrangement.subject }} <br />
          <div v-if="arrangement.location != 'Virtual'">
            <text style="font-weight: bold">Address: </text>
            {{ arrangement.address }} <br />
          </div>
          <div v-else>
            <text style="font-weight: bold">Location: </text>
            {{ arrangement.location }} <br />
          </div>
          <text style="font-weight: bold">Day: </text>
          {{ arrangement.preferredDays }} <br />
          <text style="font-weight: bold">Time: </text>
          {{ arrangement.preferredTime }}
        </div>
        <div class="buttons">
          <button
            class="chatbutton"
            @click="redirectToChat(arrangement.chatId)">
            Chat</button
          ><br />
          <button
            class="progressbutton"
            @click="redirectToProgress(arrangement.id)">
            Progress</button
          ><br />
          <button
            class="endsessionbutton"
            @click="endSession(arrangement.id, arrangement.tutorName)">
            End Session
          </button>
        </div>
      </div>
